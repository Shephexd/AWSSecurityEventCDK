...
class InventoryManagementStack(Stack):
    ...
    def init_cce_finding_flow(self, target_log_group: str):
        """
        Delievry CCE Finding results from Systems Manager to SecurityHub
        Systems Manager -> CloudWatch(TARGET_LOG_GROUP) -> Lambda(ImportCCEFindings) -> SecurityHub
    ...
    def init_cce_finding_notification(self, email, topic_name):
        ...
        email_parser = self.init_parse_findings_lambda(sns_topic=topic)
    ...
    def init_parse_findings_lambda(self, sns_topic):
        """
